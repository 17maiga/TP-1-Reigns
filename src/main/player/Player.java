...
public class Player {
    ...
    public Player() {
        ...
        for (int i = 0; i < Gender.values().length - 1; i++) {
            genderQuestion.append(" a ").append(Gender.values()[i].getTitle()).append(" (").append(i + 1).append("),");
        }
        genderQuestion.append(" or a ").append(Gender.values()[Gender.values().length - 1].getTitle()).append(" (").append(Gender.values().length).append(") ?");
        int answer = 0;
